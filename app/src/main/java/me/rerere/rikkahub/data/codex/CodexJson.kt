...
internal fun parseCodexUsage(jsonObject: JsonObject): CodexUsageSnapshot {
    val rateLimit = jsonObject["rate_limit"]?.jsonObject
    return CodexUsageSnapshot(
        primary = rateLimit?.get("primary_window")?.jsonObject?.toUsageWindow(),
        secondary = rateLimit?.get("secondary_window")?.jsonObject?.toUsageWindow(),
    )
}
...
internal fun parseCodexUsage(headers: Headers): CodexUsageSnapshot? {
    fun window(prefix: String): CodexUsageWindow? {
        ...
        return CodexUsageWindow(
            usedPercent = used,
            windowMinutes = headers["x-codex-$prefix-window-minutes"]?.toLongOrNull(),
            resetsAt = resetsAt,
        )
    }
...
private fun JsonObject.toUsageWindow(): CodexUsageWindow? {
    ...
    return CodexUsageWindow(
        usedPercent = used,
        windowMinutes = this["limit_window_seconds"]?.jsonPrimitive?.longOrNull?.div(60),
